...
export async function renderFullCharacterSheet(characterData, isModal, aspect, isInPlay, targetContainer) {
    ...
    if (characterData.relationships && characterData.relationships.length > 0) {
     ...
     const style = `
  ${!isInPlay ? '' : 'transform: scale(.9);'}
  ${origin ? origin + ';' : ''}
  background-image: url('${imageUrl}');
  background-size: cover;
  background-position: center;
  border: 1px solid ${predominantColor};
  box-shadow: 0 0 20px ${predominantColor};
  width: ${finalWidth}px;
...
`;
    ...
    const sheetHtml = `
            ...
            <div id="character-sheet" class="w-full h-full rounded-lg shadow-2xl overflow-hidden relative text-white" style="${style}">    
